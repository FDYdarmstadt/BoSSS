...
namespace BoSSS.Foundation {
    ...
    /// event used in <see cref="ISpatialOperator.ParameterUpdates"/>.
    /// </summary>
    /// <param name="DomainVarFields">
    /// Input fields, current state of domain variables.
    /// - key: variable name as defined by the operator (<see cref="ISpatialOperator.DomainVar"/>)
    ...
    /// 2. For an <em>event</em>, such as <see cref="ISpatialOperator.ParameterUpdates"/> multiple handlers can be added so it is not necessary to 
    ...
    /// - parameter name: must match one name in <see cref="ISpatialOperator.ParameterVar"/>
