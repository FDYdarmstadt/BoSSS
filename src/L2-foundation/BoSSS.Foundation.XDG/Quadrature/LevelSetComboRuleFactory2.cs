...
namespace BoSSS.Foundation.XDG.Quadrature.HMF {
    ...
    public class LevelSetComboRuleFactory2 {
        ...
        public LevelSetComboRuleFactory2(LevelSetTracker tracker, int iLevSet,
            IQuadRuleFactory<CellBoundaryQuadRule> cellBoundaryFactory,
...
//                    throw new ApplicationException("illegal call"); // rule should be in cache, request for re-computation indicates that something is funky
                ...
                var TestBasis = new Basis(this.tracker.GridDat, IntOrder);
                NoOfEqPerSy = TestBasis.Length * this.tracker.GridDat.Grid.SpatialDimension;
        ...
        protected MultidimensionalArray GaußAnsatzMatrix(Basis TestBasis, NodeSet VolQrNodes, NodeSet SurfQrNodes, int jCell, out MultidimensionalArray metrics) {
            ...
            var LevelSetNormals = this.tracker.GetLevelSetReferenceNormals(this.LevelSetIndex, SurfQrNodes, jCell, 1).ExtractSubArrayShallow(0,-1,-1);

            metrics = tracker.GetLevelSetNormalReferenceToPhysicalMetrics(this.LevelSetIndex, SurfQrNodes, jCell, 1);
