...
namespace BoSSS.Foundation.XDG.Quadrature
{
    ...
    class DoubleSayeQuadratureFactory : IQuadRuleFactory<QuadRule>
    {
        ...
        public DoubleSayeQuadratureFactory(IScheme scheme, int resolution = 200)
        {
            this.scheme = scheme;
            this.resolution = resolution;
        }

        public DoubleSayeQuadratureFactory(IQuadRuleFactory<QuadRule> fallBackFactory, LevelSetTracker.LevelSetData[] data, CombinedID id, IScheme scheme, int resolution = 200) : this(scheme, resolution) {
        ...
        public virtual IEnumerable<IChunkRulePair<QuadRule>> GetQuadRuleSet(ExecutionMask mask, int order)
        {
            ...
            foreach(Chunk chunk in mask)
            {
                for(int i = chunk.i0; i < chunk.JE; ++i)
                {
                    ...
                    if (SayeSettingsOverride.doubleCutCellOverride) {
                        if (detector != null) {
