...
//int hypre_F90_NAME_LAPACK(dgels, DGELS)(char *, int *, int *, int *, double *, int *,  double *, int *, double *, int *, int *);
#endif

#ifdef WIN32
static void bzero(char *a, int n) {
  int i;
  for (i=0; i<n; i++) {a[i]=0;}
}
#endif
  ...
  int local_beg_row, Numbering *numb, int symmetric)
{
    ...
    for (row=local_beg_row; row<=mat->end_row; row++)
    {
	    ...
	    DPOTRF(&uplo, &len, ahat, &len, &info,1);
        //extern void dpotrf(char uplo, int n, double *a, int lda, int *info);
        //dpotrf(uplo, len, ahat, len, &info);
        ...
        DPOTRS(&uplo, &len, &one, ahat, &len, val, &len, &info,1);
        //dpotrs(uplo,len,one,ahat,len,val,len,&info);
        if (info != 0)
        {
#if 0
  ...
  int local_beg_row, Numbering *numb)
{
    ...
    for (row=local_beg_row; row<=mat->end_row; row++)
    {
        ...
        DGELS(&trans, &npat, &len, &one, ahat, 
              &npat, bhat, &npat, work, &work_size,
              &info,1);
        //dgels( trans,npat,len,one,ahat,npat,bhat,

	//dgels(char trans, int m, int n, int nrhs, double *a,
