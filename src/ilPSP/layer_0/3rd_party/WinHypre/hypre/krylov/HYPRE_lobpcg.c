...
//#ifdef HYPRE_USING_ESSL
   ...
   // according to acml.h (and my own tests), the C-routines of ACML take FORTRAN arrays,
   // so this call should be o.k.
//#else
...
hypre_LOBPCGSetup( void *pcg_vdata, void *A, void *b, void *x )
{
  hypre_LOBPCGData *pcg_data = pcg_vdata;
  HYPRE_MatvecFunctions * mv = pcg_data->matvecFunctions;
  int  (/*__stdcall*/ *precond_setup)() = (pcg_data->precondFunctions).PrecondSetup;
...
hypre_LOBPCGPreconditioner( void *vdata, void* x, void* y ) {
  ...
  int (/*__stdcall*/ *precond)() = (data->precondFunctions).Precond;
