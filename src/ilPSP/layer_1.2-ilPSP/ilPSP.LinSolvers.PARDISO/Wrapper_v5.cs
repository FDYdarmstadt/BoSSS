...
namespace ilPSP.LinSolvers.PARDISO {
    ...
    /// <summary>
    /// wrapper for loading the PARDISO Version 5
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class Wrapper_v5 : DynLibLoader {


        static string [][] linux_pardiso_preqs = new string[][] { new string[] { "libm.so", "libm.so*" }, 
                                new string[] { "libacml.so", "libblas.so", "libblas.so*"},
                                new string[] { "libacml.so", "liblapack.so", "liblapack.so*" },
                                new string[] { "libgomp.so.1", "libgomp.so", "libgomp.so*" }
                };
        ...
        public Wrapper_v5()
            : base(
                new string[] { "libpardiso500-WIN-X86-64.dll", "libpardiso500-GNU*-X86-64.so"  },
                new string[][][] { null, linux_pardiso_preqs },
        ...
        /// </summary>
        public unsafe delegate void _pardisoinit(void* pt, int* mtype, int* solver, int* iparm, double* dparm, int* error);
