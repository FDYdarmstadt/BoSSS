...
namespace BoSSS.Solution.NSECommon {
    ...
    public class RHSManuSourceNS : BoSSS.Solution.Utils.LinearSource {
        ...
        protected override double Source(double[] x, double[] parameters, double[] U) {
            ...
            if (direction == "x") {
                if (!energyOK && !speciesOK) { // just conti and momentum
                    ConvectionTerm = -0.20e1 * Math.Cos(x_) * Math.Sin(x_) - 0.10e1 * Math.Cos(x_) * Math.Sin(y_); // conti and mom
                } else if (energyOK && !speciesOK) { // conti, momentum and energy
                ...
                } else if (energyOK && speciesOK) { // conti, momentum, energy and species
            ...
            return -(ConvectionTerm + ViscTerm*0 + PressureGradientTerm);
        }
    }
}
