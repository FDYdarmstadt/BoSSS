...
namespace BoSSS.Solution.AdvancedSolvers {
    ...
    public class OrthonormalizationMultigrid : ISolverSmootherTemplate, ISolverWithCallback, IProgrammableTermination, ISubsystemSolver {
                ...
                for (iIter = 1; bIterate; iIter++) {
                   WriteDebug(iIter, resNorm, "initial start");
					...
					if (PreSmoother != null && !skipPreSmoothInFollowingIters && !config.SkipPreSmoother) {
                        // Check if non-serial smoother is demanded
						double[] ResBeforePreSmoother = null, ResAfterPreSmoother = null;
						if (config.NonSerialPreSmoother) { //create temporary arrays to store current residuals
							ResBeforePreSmoother = new double[L];
							Array.Copy(Res, ResBeforePreSmoother, L);
						}

						resNorm = ApplyPreSmoothers(iIter, X, B, Res, Sol0, Res0);
						skipPreSmoothInFollowingIters = false;
						...
						if (config.NonSerialPreSmoother) {
                    ...
                    ApplyCoarseGridCorrection(f, ref resNorm, iIter, X, B, Res, ResCoarse, Sol0, Res0);
		...
		private double ApplyPreSmoothers(int iIter, double[] X, double[] B, double[] Res, double[] Sol0, double[] Res0) {
				// Be aware that we have two options for skipping the pre-smoother: one for the following iterations (determined at post smoother), the second for skipping in general from config.
				VerivyCurrentResidual(X, B, Res, iIter);
			...
			double[] PreCorr = new double[X.Length];
			PreSmoother.Solve(PreCorr, Res); // Vorglättung

			// orthonormalization and residual minimization
			double resNorm = ortho.AddSolAndMinimizeResidual(ref PreCorr, X, Sol0, Res0, Res, "presmoothL" + iLevel);
        ...
        private void ApplyCoarseGridCorrection(FuncTrace f, ref double resNorm, int iIter, double[] X, double[] B, double[] Res, double[] ResCoarse, double[] Sol0, double[] Res0) {
			CrseLevelTime.Start();
			// Test: Residual on this level / already computed by 'MinimizeResidual' above
			VerivyCurrentResidual(X, B, Res, iIter);
			...
			for (int i = 0; i < myConfig.m_omega; i++) {
				if (this.CoarserLevelSolver != null) {
					...
					if (myConfig.CoarseOnLowerLevel) {
						...
						double[] vlc = new double[ResCoarse.Length];
        ...
        private double ApplyPostSmoothers(ISolverSmootherTemplate[] allSmooters, FuncTrace f, int iIter, int iPostSmooter, double[] X, double[] B, double[] Res, double[] Sol0, double[] Res0) {
			bool termPost = false;
			...
			for (int g = 0; g < config.NoOfPostSmootherSweeps; g++) { // Test: Residual on this level / already computed by 'MinimizeResidual' above
				...
				if (iPostSmooter > 0)
					InitAdditionalPostSmooters();
				VerivyCurrentResidual(X, B, Res, iIter); // 
				double[] PostCorr = new double[X.Length];
