...
namespace BoSSS.Solution.AdvancedSolvers {
	...
	public class TaskParallelOrthoMG : ISolverSmootherTemplate, ISolverWithCallback, IProgrammableTermination, ISubsystemSolver {
        ...
        private void InitiateCoarsestSolver() {
			...
			//TestMatrices(CoarserTpMapping.OperatorMatrix, coarsePermutationMtx, subCommCoarseOpMatrix, $"test_lvl{TpLevel}_coarse_", true);
		...
		private void PerformIterations(double[] X, double[] B, double[] Res, double[] Res0) {
			...
			for (iIter = 1; ; iIter++) {
				...
				if (myTask == TpTaskType.All || myTask == TpTaskType.Coarse) {
					...
					while (!done) {
						Console.WriteLine($"{k}-Coarse Waiting for signal on rank {thisCommRank} from {ThisSmootherGroupLeader} on {thisComm} with size {thisCommsize} on TpLevel{TpLevel}");
						Thread.Sleep(1000);
						k++;
						done = CheckSignal();
					}
				...
				if (myTask == TpTaskType.All || myTask == TpTaskType.Smoother) {
					...
					while (!done) {
						Console.WriteLine($"{k}-Smoother Waiting for signal on rank {thisCommRank} from {ThisCoarseGroupLeader} on {thisComm} with size {thisCommsize} on TpLevel{TpLevel}");
						Thread.Sleep(1000);
						k++;
						done = CheckSignal();
					}
