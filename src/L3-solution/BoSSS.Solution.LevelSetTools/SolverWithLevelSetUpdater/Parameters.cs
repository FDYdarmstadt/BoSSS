...
namespace BoSSS.Solution.LevelSetTools.SolverWithLevelSetUpdater {
    ...
    public class Normals : ParameterS, ILevelSetParameter {
        ...
        public Normals(int D, int degree) : this(VariableNames.LevelSetCG, D, degree) {
        }
        ...
        public Normals(string LsName, int D, int degree) {
            ...
            if (LsName == VariableNames.LevelSetCG) {
                parameterNames = VariableNames.NormalVector(D);
            } else {
    ...
    public class GradientAndCurvature : ParameterS, ILevelSetParameter {
        ...
        public GradientAndCurvature(string LsName, int curvatureDegree, int gradientDegree, int m_HMForder, int D, SurfaceStressTensor_IsotropicMode mode, CurvatureAlgorithms.FilterConfiguration filter) {
            if (LsName == VariableNames.LevelSetCG) {
        ...
        public override IList<string> ParameterNames => new[] { lsParameters.Last()}; // the operator only uses the curvature!
        ...
        public (string ParameterName, DGField ParamField)[] ParameterFactory(IReadOnlyDictionary<string, DGField> DomainVarFields) {
            ...
            fields[0] = (this.ParameterNames[0], new SinglePhaseField(curvatureBasis, this.ParameterNames[0]));
            return fields;
        }
