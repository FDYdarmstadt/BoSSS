...
namespace BoSSS.Solution.XNSECommon.Operator.Viscosity {
    ...
    /*
    /// <summary>
    /// Extension form, used with <see cref="ViscosityAtLevelSet_FullySymmetric"/>, without slip included there. Proofed to be badly conditioned. Instead implement interface slip as Neumann type B.C.!
    /// </summary>
    public class ViscositySlipAtLevelSet_FullySymmetric : BoSSS.Foundation.XDG.ILevelSetForm, ILevelSetEquationComponentCoefficient, ISupportsJacobianComponent {

        //LevelSetTracker m_LsTrk;

        public ViscositySlipAtLevelSet_FullySymmetric(int SpaceDim, double _sI, double _muA, double _muB, double _penalty, int _component, 
        ...
        public double InnerEdgeForm(ref CommonParams inp,
            ...
            // corrections apply only to symmetry and penalty term
            if (!freeSurface) {
                for(int j = 0; j < D; j++) {
                    //Ret += P[component, j] * (uA[j] - uB[j]) * (vA - vB);
                    for(int k = 0; k < D; k++) {
                        //Ret += 1/beta * P[component, j] * (wA * muA * Grad_uA[j,k] + wB * muB * Grad_uB[j,k]) * N[k] * (vA - vB);
                        double t = -1/beta * (wA * muA * Grad_uA[k,j] + wA * muA * Grad_uA[j,k] + wB * muB * Grad_uB[k,j] + wB * muB * Grad_uB[j,k]) * N[j];
                        Ret += P[component, k] * t * (wA * muA * Grad_vA_xN + wB * muB * Grad_vB_xN); // symmetry
                        Ret -= P[component, k] * t * (vA - vB) * pnlty * wPenalty; // penalty term
                        for(int i = 0; i < D; i++) {
                             Ret += P[i, k] * t * (wA * muA * Grad_vA[i] + wB * muB * Grad_vB[i]) * N[component]; // symmetry-transpose
                        }
                    }
                }
            } else {
