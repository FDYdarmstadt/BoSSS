    ...
    class NavierCauchy : BulkEquation {
        ...
        public NavierCauchy(string speciesName, Solid material, int d, int D, IncompressibleMultiphaseBoundaryCondMap boundaryMap) {
            ...
            if(material.Lame2 != 0.0) {
                var eulerAlmansi0 = new SIPForm(SpeciesName, HFSISolver.VariableNames.DisplacementVector(D), d, material.Lame2, boundaryMap, 1);
                AddComponent(eulerAlmansi0);
                var eulerAlmansi1 = new SIPTransposeForm(SpeciesName, HFSISolver.VariableNames.DisplacementVector(D), d, material.Lame2, boundaryMap, 0);
                AddComponent(eulerAlmansi1);
                //AddComponent(new SIPGradUTGradUForm(SpeciesName, HFSISolver.VariableNames.DisplacementVector(D), d, -material.Lame2, boundaryMap, 0));
                ...
                //AddComponent(new EdgePenaltyForm(SpeciesName, HFSISolver.VariableNames.DisplacementVector(D)[d], material.Lame2));
            }
            if(material.Viscosity != 0.0)
            {
                var viscosity = new SIPForm(SpeciesName, BoSSS.Solution.NSECommon.VariableNames.VelocityVector(D), d, material.Viscosity, boundaryMap, 1);
                AddComponent(viscosity);
                var viscosityT = new SIPTransposeForm(SpeciesName, BoSSS.Solution.NSECommon.VariableNames.VelocityVector(D), d, material.Viscosity, boundaryMap, 0);
