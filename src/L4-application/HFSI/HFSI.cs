...
namespace HFSISolver {
    ...
    public class HFSI : XNSFE<HFSI_Control> {
        ...
        private void AddXDisplacementMultigridConfigLevel(List<MultigridOperator.ChangeOfBasisConfig> configsLevel) {
        ...
        protected override void AddMultigridConfigLevel(List<MultigridOperator.ChangeOfBasisConfig> configsLevel, int iLevel) {
            ...
            AddXDisplacementMultigridConfigLevel(configsLevel);
        }
        ...
        protected override void AddMultigridConfigLevel(List<MultigridOperator.ChangeOfBasisConfig> configsLevel, int iLevel) {
            ...
            var configTemp = new MultigridOperator.ChangeOfBasisConfig() {
                DegreeS = new int[] { pTemp },
                mode = MultigridOperator.Mode.IdMass,
                VarIndex = new int[] { this.XOperator.DomainVar.IndexOf(NSEVariableNames.Temperature) }
            };
            configsLevel.Add(configTemp);
            // configuration for displacements
            for(int d = 0; d < D; d++) {
                var configDisplacement = new MultigridOperator.ChangeOfBasisConfig() {
                    DegreeS = new int[] { pDispl },
        ...
        */
        ...
        protected override void DefineSystemImmersedBoundary(int D, OperatorFactory opFactory, LevelSetUpdater lsUpdater) {
            ...
            //opFactory.AddEquation(new SolidHeat("C", D, thermBoundaryMap, config));//heat equation in solid without convection term
            if(this.Control.NonLinearSolver.SolverCode == NonLinearSolverCode.Picard) {
                opFactory.AddEquation(new Heat("C", D, thermBoundaryMap, config));
            } else if(this.Control.NonLinearSolver.SolverCode == NonLinearSolverCode.Newton) {
                opFactory.AddEquation(new Heat_Newton("C", D, thermBoundaryMap, config));
            } else {
                throw new NotSupportedException();
            }
            ...
            //opFactory.AddEquation(new Dummy("C", NSEVariableNames.Temperature, NSEEquationNames.HeatEquation));

            //*
            if(config.dntParams.SST_isotropicMode == BoSSS.Solution.LevelSetTools.SurfaceStressTensor_IsotropicMode.LaplaceBeltrami_ContactLine) {
                for(int d = 0; d < D; ++d) {
