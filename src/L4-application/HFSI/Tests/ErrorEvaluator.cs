...
namespace HFSISolver.Tests
{
    ...
    public abstract class ZLSErrorEvaluator {
        ...
        /// </summary>
        public abstract double[] ComputeL2Error(double time, HFSI_Control control);
    ...
    /// </remarks>
    public class ZLSErrorEvaluator<T> : ZLSErrorEvaluator where T: HFSI_Control, new() {
        ...
        public double[] ComputeVelocityError(IDictionary<string, Func<double[], double, double>[]> exactVelocity, double time)
        {
            ...
            foreach (var spc in FluidSpecies)
            {
                ...
                for (int d = 0; d < D; d++)
                {
                    ...
                    solver.QueryHandler.ValueQuery("L2err_" + BoSSS.Solution.NSECommon.VariableNames.Velocity_d(d) + "#" + spc, L2Error_Species[spc][d], true);
                }
            }
        ...
        /// </summary>
        public override double[] ComputeL2Error(double time, HFSI_Control control) {
            int D = solver.GridData.SpatialDimension;
            //double[] Ret = new double[D + 1];
            double[] Ret = new double[D + 1 + 2 + 1]; //Needs to be improved
    ...
    /// </summary>
    public class LevelSetErrorEvaluator<T> : ZLSErrorEvaluator where T: HFSI_Control, new() {
        ...
        public double ComputeLevelSetError(CellMask cm) {
            ...
            SinglePhaseField PhiCG = solver.LsUpdater.LevelSets[BoSSS.Solution.NSECommon.VariableNames.LevelSetCG].C0LevelSet;
        ...
        public double ComputeDGLevelSetError(CellMask cm) {
            ...
            SinglePhaseField PhiDG = solver.LsUpdater.LevelSets[BoSSS.Solution.NSECommon.VariableNames.LevelSetCG].DGLevelSet;
