...
namespace HFSISolver.Tests {
    ...
    /// </summary>
    public class FSTaylorCouette : IHFSITest {
        ...
        internal class ExactSol {
            ...
            /// <summary>
            /// pressure, phase C, radial coordinates
            /// </summary>
            ...
            /// <summary>
            /// temperature, phase A, radial coordinates
            /// </summary>
            public double tA(double r) {
                return 8.600209680055716 * Math.Log(r) - 0.961211091954969;
            }

            /// <summary>
            /// temperature, phase B, radial coordinates
            /// </summary>
            public double tB(double r) {
                return 0;
            }

            /// <summary>
            /// temperature, phase C, radial coordinates
            /// </summary>
            public double tC(double r) {
                return 0.645015726004179 * Math.Log(r) + 1.447090831896623;
            }
            ...
            public double TA(double[] X, double t) {
                double p = tA(X.L2Norm());
                if(p.IsNaNorInf())
                    throw new ArithmeticException();
                return p;
            }

            public double TB(double[] X, double t) {
                double p = tB(X.L2Norm());
                if(p.IsNaNorInf())
                    throw new ArithmeticException();
                return p;
            }

            public double TC(double[] X, double t) {
                double p = tC(X.L2Norm());
                if(p.IsNaNorInf())
                    throw new ArithmeticException();
                return p;
            }
        ...
        public double[] AcceptableL2Error => new double[] { 1, 1, 1, 1, 1, 1 };//Needs to be improved

        public double[] AcceptableResidual => new double[] { 1, 1, 1, 1, 1, 1 };//Needs to be improved
        ...
        public Func<double[], double, double> GetTemperature(string species) {
            switch(species) {
                case "A": return exS.TA;
                case "B": return exS.TB;
                case "C": return exS.TC;
                default: throw new ArgumentOutOfRangeException();
            }
        }
