...
namespace FSI_Solver {
    class FSI_Collision {
        ...
        private List<List<int>> ParticleCollidedWith = new List<List<int>>();
        private int[] ParticleNoOfCollisions;
        private double[][] TemporaryVelocity;
        private Particle[] Particles;
        private List<List<int>> ParticleCluster = new List<List<int>>();
        private List<int[]> ParticleClusterCollidedWithWall = new List<int[]>();
        ...
        public FSI_Collision(double gridLenghtscale, double coefficientOfRestitution, double dt, double[][] wallCoordinates, double minDistance) {
            CoefficientOfRestitution = coefficientOfRestitution;
            Dt = dt;
            GridLengthScale = gridLenghtscale;
        ...
        private void CreateCollisionArrarys(int noOfParticles) {
            ...
            ParticleNoOfCollisions = new int[noOfParticles + 4];
                        ...
                        for (int p1 = 1; p1 < ParticleCollidedWith[p0].Count(); p1++) {
                            if (!PartOfCollisionCluster[p0]) {
                                ...
                                if (ParticleCollidedWith[p0][p1] < particles.Length) {
                                    ...
                                    FindCollisionClusterRecursive(ParticleCollidedWith, p1, ParticleCluster.Last());
                                }
                                else {
                            ...
                            for (int i = 0; i < ParticleCluster[c].Count(); i++) {
                                ...
                                for (int j = 1; j < ParticleCollidedWith[currentParticleID].Count(); j++) {
                                    ...
                                    if (secondParticleID < particles.Length) {
                                        ...
                                        ComputeMomentumBalanceCollision(currentParticleID, secondParticleID, normalVector);
                                    }
                                    else {
                                        Vector normalVector = DistanceVector[currentParticleID][secondParticleID] / (DistanceVector[currentParticleID][secondParticleID]).Abs();
                                        particles[currentParticleID].ClosestPointToOtherObject = ClosestPoints[currentParticleID][secondParticleID];
...
;
        ...
        /// </param>
        internal void ComputeMomentumBalanceCollision(int p0, int p1, Vector normalVector) {
            ...
            Vector tempVel0 = Particles[p0].Motion.IncludeTranslation 
                ? (velocityP0[0] - collisionCoefficient / Particles[p0].Motion.Mass_P) * normalVector + velocityP0[1] * CoefficientOfRestitution * tangentialVector 
                : new Vector(0, 0);
            Vector tempVel1 = Particles[p1].Motion.IncludeTranslation
                ? (velocityP1[0] + collisionCoefficient / Particles[p1].Motion.Mass_P) * normalVector + velocityP1[1] * CoefficientOfRestitution * tangentialVector
                : new Vector(0, 0);
        ...
        private double CalculateCollisionCoefficient(int p0, int p1, Vector normalVector) {
            ...
            for (int p = 0; p < 2; p++) {
                ...
                massReciprocal[p] = Particles[currentParticleIndex].Motion.IncludeTranslation ? 1 / Particles[currentParticleIndex].Motion.Mass_P : 0;
        ...
        private double CalculateCollisionCoefficient(int p0, Vector normalVector) {
            ...
            Vector velocityP0 = CalculateNormalAndTangentialVelocity(p0, normalVector);
            double collisionCoefficient = (1 + CoefficientOfRestitution) * (velocityP0[0] / (1 / currentParticle.Motion.Mass_P + currentParticle.Eccentricity.Pow2() / currentParticle.MomentOfInertia));
            collisionCoefficient += -(1 + CoefficientOfRestitution) * currentParticle.Eccentricity * TemporaryVelocity[p0][2] / (1 / currentParticle.Motion.Mass_P + currentParticle.Eccentricity.Pow2() / currentParticle.MomentOfInertia);
            return collisionCoefficient;
        }
