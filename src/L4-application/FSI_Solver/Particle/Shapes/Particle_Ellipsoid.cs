...
namespace BoSSS.Application.FSI_Solver {
    ...
    public class Particle_Ellipsoid : Particle {
        ...
        public override double phi_P(double[] X) {
            double alpha = -(angleAtTimestep[0]);
            var r = 
               -(((X[0] - positionAtTimestep[0][0]) * Math.Cos(alpha) - (X[1] - positionAtTimestep[0][1]) * Math.Sin(alpha)).Pow2() / length_P.Pow2()) 
               -(((X[0] - positionAtTimestep[0][0]) * Math.Sin(alpha) + (X[1] - positionAtTimestep[0][1]) * Math.Cos(alpha)).Pow2() / thickness_P.Pow2()) 
               + 1.0;
            if (double.IsNaN(r) || double.IsInfinity(r))
                throw new ArithmeticException();
            return r;
