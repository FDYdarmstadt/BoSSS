...
namespace BoSSS.Application.FSI_Solver {
    ...
    public class Particle_superEllipsoid : Particle {
        ...
        override public MultidimensionalArray GetSurfacePoints(LevelSetTracker lsTrk)
        {
            ...
            {
                SurfacePoints[j, 0] = Math.Pow(Math.Cos(InfinitisemalAngle[j]), 2 / superEllipsoidExponent) * length_P + Position[0][0];
                SurfacePoints[j, 1] = Math.Pow(Math.Sin(InfinitisemalAngle[j]), 2 / superEllipsoidExponent) * thickness_P + Position[0][1];
            }
