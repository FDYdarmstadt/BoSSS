        ...
        public void CheckSolutionR0Compatibility(SinglePhaseField ur, SinglePhaseField uxi, SinglePhaseField ueta, SinglePhaseField Pressure, bool throwException = true) {
            ...
            for(int varIndex = 0; varIndex < helicalSol.NoOfVariables; varIndex++) {
                ...
                if(varIndex < 2) {
                    ...
                    if(throwException) {
                        Assert.Less(result_in.Max().MPIMax().Abs() / Globals.MaxAmp, 1E-14, "R0fix_exeption: Max value for {0} should be less than {2} but is {1} (Devided by Max Amplitude)", variables[varIndex], result_in.Max() / Globals.MaxAmp, 1E-14 );
                        Assert.Less(result_in.Min().MPIMin().Abs() / Globals.MaxAmp, 1E-14, "R0fix_exeption: Min value for {0} should be less than {2} but is {1} (Devided by Max Amplitude)", variables[varIndex], result_in.Min() / Globals.MaxAmp, 1E-14 );
                    }
                } else if(varIndex >= 2) {
