...
namespace ZwoLevelSetSolver.ControlFiles {
    ...
    public static class HardCodedControl {
        ...
        public static ZLS_Control Test_VerticalBeamInChannel(int p = 2, int kelem = 50, int AMRlvl = 0) {
            ZLS_Control C = new ZLS_Control(p);
            C.ImmediatePlotPeriod = 1;
            C.SuperSampling = 4;
            ...
            Func<double[], double> Phi1Func = delegate (double[] X) {
                return -(Math.Pow(Math.Abs(X[0] / a), power) + Math.Pow(Math.Abs(X[1] / b), power)) + 1;
