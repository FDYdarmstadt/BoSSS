...
namespace BoSSS.Application.XNSEC {
    ...
    public static partial class FullNSEControlExamples {
        ...
        /// <returns></returns>
        static public XNSEC_Control FS_XDG_Droplet_2dEvaporation(int DGp = 2, int nCells = 6, string dbPath = null) {
            var C = XDG_Droplet_2dEvaporation(DGp, nCells, dbPath, true); // true for MF calculation
        ...
        /// <param name="dbPath"></param>
        /// <returns></returns>
        static public XNSEC_Control Full_XDG_Droplet_2dEvaporation(int DGp = 2, int nCells = 6, string dbPath = null) {

            var C = XDG_Droplet_2dEvaporation(DGp, nCells, dbPath, false); // if false, we have combustion
        ...
        public static XNSEC_Control XDG_Droplet_2dEvaporation(int DGp = 2, int nCellsMult = 6, string dbpath = null, bool MF = false) {
            XNSEC_Control C;
            ...
            foreach (var spc in new[] { "A", "B" }) {
                ...
                for (int d = 0; d < D; d++) {
                    ...
                    C.AddInitialValue(VariableNames.Velocity_d(d), spc, GetU(spc, d).Convert_Xt2X(0.0));
                }

                C.AddInitialValue(VariableNames.Pressure, spc, GetPress(spc).Convert_Xt2X(0.0));
                C.AddInitialValue(VariableNames.MixtureFraction, spc, X => 1.0);
                C.AddInitialValue(VariableNames.Temperature, spc, GetTemperature(spc).Convert_Xt2X(0.0));
                C.AddInitialValue(VariableNames.MassFraction0, spc, X => 1.0);
                C.AddInitialValue(VariableNames.MassFraction1, spc, X => 0.0);
                C.AddInitialValue(VariableNames.MassFraction2, spc, X => 0.0);
                C.AddInitialValue(VariableNames.MassFraction3, spc, X => 0.0);
            }
