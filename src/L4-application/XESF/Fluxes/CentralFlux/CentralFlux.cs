...
namespace XESF.Fluxes {
    public abstract class CentralFlux : IEdgeForm, IVolumeForm, ISupportsJacobianComponent {
        ...
        /// </param>
        public CentralFlux(IBoundaryConditionMap boundaryMap, Material material) {
        ...
        public double InnerEdgeForm(ref CommonParams inp, double[] _uIN, double[] _uOUT, double[,] _Grad_uIN, double[,] _Grad_uOUT, double _vIN, double _vOUT, double[] _Grad_vIN, double[] _Grad_vOUT) {
            ...
            int D = inp.D;
            return 0.5 * (GetFluxVector(D, _uIN) * inp.Normal + GetFluxVector(D, _uOUT) * inp.Normal) * (_vIN - _vOUT);
            //return GetFluxVector(D, U) * inp.Normal * (_vIN - _vOUT);
        }
        public double VolumeForm(ref CommonParamsVol cpv, double[] U, double[,] GradU, double V, double[] GradV) {
            return -1.0 * GetFluxVector(cpv.D, U) * GradV;
        }
    }
}
