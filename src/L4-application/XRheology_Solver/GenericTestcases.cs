...
namespace BoSSS.Application.XRheology_Solver {
    ...
    public static class GenericTestcases {
        ...
        /// </summary>
        /// <returns></returns>
        public static XRheology_Control InterfaceTest(int p = 2, int kelem = 8, int wallBC = 0) {
            ...
            C.GridFunc = delegate () {
                double[] Xnodes = new double[] {0, 1, 2, 3};
                double[] Ynodes = new double[] { 0, 1, 3 };
            ...
            Func<double[], double> PhiFunc = (X => X[1]-2); // + (H/20)*Math.Cos(8 * Math.PI * X[0] / L)); //-1);
