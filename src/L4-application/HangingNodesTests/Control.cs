...
namespace HangingNodesTests {
    partial class Control {
        ...
        /// <param name="setup"></param>
        /// <returns></returns>
        static public XNSFE_Control TestSkeletonShear(double size) {
            var ctrl = new XNSFE_Control();
            ...
            ctrl.AddBoundaryValue("wall_ConstantTemperature_left", "VelocityY#C", $"(X, t) => -1000*{L}", true);
            ...
            ctrl.AddBoundaryValue("wall_ConstantTemperature_right", "VelocityY#C", $"(X, t) => 1000*{L}", true);
            ctrl.AddBoundaryValue("pressure_outlet_ZeroGradient_upper");
            ctrl.AddBoundaryValue("pressure_outlet_ZeroGradient_lower");

            ctrl.AddInitialValue("VelocityY#A", $"(X, t) => 1000*(X[0])", true);
            ctrl.AddInitialValue("VelocityY#B", $"(X, t) => 1000*(X[0])", true);
            ctrl.AddInitialValue("VelocityY#C", $"(X, t) => 1000*(X[0])", true);
