        ...
        */
        ...
        public void WriteLogLine(TimestepNumber TimestepNo, double phystime) {
            ...
            switch (this.Control.LogValues) {
                ...
                case XNSE_Control.LoggingValues.RisingBubble: {
                        ...
                        string line = String.Format("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}", TimestepNo, phystime, BmQ_RB[0], BmQ_RB[1], BmQ_RB[2], BmQ_RB[4], BmQ_RB[5]);
                        Log.WriteLine(line);
                        Log.Flush();
        ...
        public double GetContactLineLength() {
            ...
            if(this.LsTrk.GridDat.SpatialDimension == 3) {
                ...
                System.Collections.BitArray lowerBits = new System.Collections.BitArray(this.GridData.Edges.Count);
                foreach(Chunk cnk in boundaryCutEdge) {
                    for(int iE = cnk.i0; iE < cnk.JE; iE++) {
                        if(this.GridData.Edges.EdgeTags[iE] == 1) {
                            lowerBits[iE] = true;
                        }
                    }
                }
            ...
            using (new FuncTrace()) {
                ...
                switch (this.Control.Option_LevelSetEvolution) {
                    ...
                    case LevelSetEvolution.ExtensionVelocity: {
                            ...
                            CellMask Accepted;
                            if(this.Control.AdaptiveMeshRefinement) {
                                int NoCells = this.GridData.Cells.NoOfCells;
                                System.Collections.BitArray Refined = new System.Collections.BitArray(NoCells);
                                for(int j = 0; j < NoCells; j++) {
                                    if(this.GridData.Cells.GetCell(j).RefinementLevel > 0)
                                        Refined[j] = true;
                                }
