...
namespace IntersectingLevelSetTest {
	...
	[Values(3, 4, 5)] int DGdegree) {
		...
		public static void Convergence2DTestAlgoim(
			...
			int i = 0;
			foreach (int cell in numbersOfCells) {
				Func<double, double, double, double> levelSet0 = (x, y, t) => (x * x + y * y - 0.125);
				Func<double, double, double, double> levelSet1 = (x, y, t) => (x + y * y - 0.25);
				var C = new TestControl(levelSet0, levelSet1);
				C.Dimension = 2;
				C.Resolution = cell + 1; //number of nodes per line
				C.NoOfTimesteps = 1;
	...
	[Values(2, 3)] int DGdegree) {
			...
			//BoSSS.Solution.Application.DeleteOldPlotFiles();
			//Func<double, double, double, double, double> levelSet3D_0 = (x, y, z, t) => (x - 1 * (Math.Tan(t / 90.1 * Math.PI) * y) + t / 90.1 * Math.PI * 0.01 * Math.Sin(t / 90.1 * Math.PI * 10));
			Func<double, double, double, double, double> levelSet3D_0 = (x, y, z, t) => (x - 2*y - z*t/10);


			//Func<double, double, double, double, double> levelSet3D_0 = (x, y, z, t) => (x - 1 * y + z*z);
			Func<double, double, double, double, double> levelSet3D_1 = (x, y, z, t) => (-y+x/100+z/100);
			var C = new TestControl();
			C.Dimension = 3;
			C.LevelSet3D_0 = levelSet3D_0;
			C.LevelSet3D_1 = levelSet3D_1;
	...
	[Values(2, 3)] int DGdegree) {
			...
			//BoSSS.Solution.Application.DeleteOldPlotFiles();
			Func<double, double, double, double, double> levelSet3D_0 = (x, y, z, t) => (x + 1 * y * y * 10 * Math.Sin(t / 90.1 * Math.PI));
			Func<double, double, double, double, double> levelSet3D_1 = (x, y, z, t) => -(y+x/100);
			var C = new TestControl();
			C.Dimension = 3;
			C.LevelSet3D_0 = levelSet3D_0;
			C.LevelSet3D_1 = levelSet3D_1;
