...
/// Previously, we define the exact solution $c_{Excat}(x,y,t)$ and the scalar components of the velocity field $\vec{u}$ as functions, 
...
\BoSSSexe
\BoSSScmd{
/// There should be now two plot-files in your current directory. Those can be opened by any standard viewer for .plt-files.
 }
\BoSSSexe
\BoSSScmd{
...
/// the current changerate for the explicit Euler scheme. In the next section we will evaluate the operator 
