...
\btab \btab return (byte)1; // wall at y = -2\newline 
\btab if (Math.Abs(X[1] - (+2.1 )) <= 1.0e-8)\newline 
\btab \btab return (byte)1; // wall at y = +2\newline 
\btab if (Math.Abs(X[0] - (-2)) <= 1.0e-8)\newline 
\btab \btab return (byte)2; // velocity inlet at x = -2\newline 
\btab if (Math.Abs(X[0] - (+20.0)) <= 1.0e-8)\newline 
\btab \btab return (byte)3; // pressure outlet at x = +20\newline 
...
\BoSSSexe
\BoSSScmd{
/// Phi is a level set variable which is not used in this simulation.
/// Therefore it has to be set to -1.
...
/// We will simulate the steady state solution by using just one timestep of 
/// $10^20$ (\code{1E20}) seconds of physical time for this instationary 
/// simulation.
...
/// An inline run wil block \BoSSSpad until the solver exits.
c.Run();
 }
\BoSSSexe
\BoSSScmd{
