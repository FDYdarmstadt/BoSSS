...
\BoSSSexe
\BoSSScmd{
/// Here, the degree for each field is set (degree k for velocity, k-1 for pressure). First, Phi is not going to be used in this simulation but it has to be specified anyways.
...
\BoSSSexe
\BoSSScmd{
/// Some information like the console output or a log containing various physical values can be found in the SessionDirectory
...
\BoSSSexe
\BoSSScmd{
/// Furthermore, the cylinder immmersed the fluid should be described
...
\BoSSSexe
\BoSSScmd{
/// As soon as we have exchanged the grids and the definition of phi. Example Control files for both,
/// the Channel and the flow around a cylinder can be found in the ControlExample directory. As soon as we ran 
/// the simulation again we can take a look at the plots and the PhysicalData file in the SessionDirectory. 
